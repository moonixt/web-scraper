...
```console
katana -h
./katana -u https://example.com -jsonl -d 1 -c 10 -headless -o example.jsonl
scrapy crawl page -a katana_file="example.jsonl" 
...
This katana fork includes integrated web scraping capabilities using Scrapy spiders. The workflow combines katana's discovery features with Scrapy's extraction power for comprehensive data collection.

### Two-Step Process

The web scraping process follows a two-step approach:

1. **Discovery Phase**: Use katana to discover and map URLs
2. **Extraction Phase**: Use Scrapy spiders to extract detailed data from discovered URLs

### Example Workflow

#### Step 1: URL Discovery with Katana
```bash
./katana -u https://ederin.net -jsonl -d 1 -c 10 -headless -o boa.jsonl
```

**Parameters explained:**
- `-u https://ederin.net` - Target URL to crawl
- `-jsonl` - Output in JSON Lines format for easy processing
- `-d 1` - Maximum crawling depth (1 level)
- `-c 10` - Concurrency (10 simultaneous requests)
- `-headless` - Use headless browser for JavaScript-heavy sites
- `-o boa.jsonl` - Output file name

#### Step 2: Data Extraction with Scrapy
```bash
scrapy crawl page -a katana_file="boa.jsonl"
```

**Parameters explained:**
- `page` - Name of the Scrapy spider
- `-a katana_file="boa.jsonl"` - Pass the katana output file as argument to spider

### Output

After completing both steps, you'll have:
- **Discovered URLs**: All accessible pages and endpoints from the target site
- **Extracted Data**: Detailed information from each page including:
  - Metadata (title, description, keywords)
  - Content structure
  - Forms and input fields
  - Links and relationships
  - Any custom data defined in your spider

### Use Cases

- **SEO Analysis**: Extract metadata, titles, and descriptions for optimization
- **Content Auditing**: Map site structure and content inventory
- **Security Testing**: Discover endpoints and analyze form inputs
- **Competitive Analysis**: Understand competitor site structure and content
- **Data Mining**: Collect structured data from web properties

### Custom Spiders

You can create custom Scrapy spiders to extract specific data types by extending the base spider and defining your extraction rules in the `tutorial/spiders/` directory.
